...
# the graph looks fine we wont be removing anything 
...
adonis2(scaled_sorensen_dist ~ cave, data = metadata_df)
adonis2(scaled_bray_dist ~ cave, data = metadata_df)
adonis2(scaled_wUnifrac_dist ~ cave, data = metadata_df)
```
...
Note that there is a weird arch or horseshoe-effect in the data. This is likely because we have complete species turnover between our caves. If you have this in your data, please take a look at the paper by [Morton et al., 2017](https://journals.asm.org/doi/10.1128/msystems.00166-16). More on this in NMDS...
...
bray_cave_pcoa <- 
  ...
  scale_color_manual(values = c(cave_colors)) + 
  theme_bw()
bray_cave_pcoa
```
...
adonis2(scaled_sorensen_dist ~ cave * date * fraction, data = metadata)
adonis2(scaled_bray_dist ~ cave * date * fraction, data = metadata)
# Note that the ORDER MATTERS!
adonis2(scaled_wUnifrac_dist ~ cave * date * fraction, data = metadata)
adonis2(scaled_wUnifrac_dist ~ date * cave * fraction, data = metadata)
```
